...
def consolidateData(paths_to_avg_data_over):
    ...
    for path in paths_to_avg_data_over:
        ...
        column_to_add = d[0:min_num_samples, INDEX_TO_PLOT]
        mid_idx = int(round(column_to_add.shape[0] / 2.0))
        if not INCLUDE_THRESHOLD or sum(column_to_add[mid_idx:]) / (column_to_add.shape[0] - mid_idx) > THRESHOLD:
            data[0:min_num_samples] += d[0:min_num_samples]
            data_statistics[:, _idx] = column_to_add
            _idx += 1

    # Delete columns if some of the data did not pass the threshold
    if INCLUDE_THRESHOLD and _idx < data_statistics.shape[1]:
        data_statistics = np.delete(data_statistics, range(_idx, data_statistics.shape[1]), 1)
    data /= len(paths_to_avg_data_over)
    print("DATA STATS: {0}".format(data_statistics.shape))
    print("Number of files that satisfy threshold: {0}".format(_idx))

    return _idx, data_statistics
...
if not RAW_PLOT:
    ...
    for operation in args:
        print("BUG CHECKING: structured_data shape {0}".format(structured_data.shape))
        print("BUG CHECKING: num_result_paths {0}".format(num_result_paths))
    ...
    for file in paths:
        ...
        try:
            d = data[:, INDEX_TO_PLOT]
            mid_idx = int(round(d.shape[0] / 2.0))
            if not INCLUDE_THRESHOLD or sum(d[mid_idx:]) / (d.shape[0] - mid_idx) > THRESHOLD:
                if PLOT_LOG:
                    d = np.log(np.abs(d))
                    # d = d**2
                    print(d)
                p, = ax.plot(d)
                plotted_objects.append(p)
        except IndexError:
        ...
        if "--baseline" in args:
            baseline_response = optimal_control_response()
            p, = ax.plot(baseline_response[:, 0], baseline_response[:, 1], "k--")
            plotted_objects.append(p)
        ...
        if SAVE_TO_FILENAME is not None:
            data = data[:, INDEX_TO_PLOT]
            to_save = np.ones([max(data.shape), 2])
            to_save[:, 0] = range(0, max(data.shape))
            to_save[:, 1] = data[:]
            np.savetxt(SAVE_TO_FILENAME, to_save, delimiter="\t")
